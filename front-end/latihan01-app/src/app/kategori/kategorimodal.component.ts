...
@Component({
  ...
  templateUrl: './kategorimodal.component.html',
})
export class KategorimodalComponent implements OnInit{

    @Output() sendDataToProduct = new EventEmitter<String>();
...
dtTrigger: Subject<any> = new Subject();
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      columns: [{
        ...
        render: function(data, type, row){
          return  `<button class="btn btn-info btn-sm pop" data-element-id="${row.categoryId}|${row.categoryName}">
          <i class="fa fa-search"></i>Choose</button>
          `;
        }
      }],
    ...
    document.querySelector('body').addEventListener('click', (event) => {
      ...
      if (target.tagName.toLowerCase() === 'button' && $(target).hasClass('pop')) {
